...
func recibir_daño(cantidad, ignorar_invulnerabilidad := false):
	...
	# Si NO ignora invulnerabilidad → daño normal
	...
	# SIEMPRE parpadea, aunque ignore invulnerabilidad
	activar_invulnerabilidad()

	# Solo activa invulnerabilidad real si NO se ignora
	if not ignorar_invulnerabilidad:
		invulnerable = true
...
func activar_invulnerabilidad():
	...
	# Solo se desactiva si NO es daño del veneno
	if invulnerable:
		invulnerable = false
